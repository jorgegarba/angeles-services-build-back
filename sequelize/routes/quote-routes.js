...
quoteRouter.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { customer: { dni }, quote: { dpto_id }, } = req.body;
    ...
    try {
        ...
        if (!customer) {
            ...
            const quoteFound = yield Quote_1.Quote.findOne({
                where: { customer_id: customer.id, dpto_id },
            });
            if (quoteFound) {
                throw new Error('Ya existe una cotización para este cliente y departamento');
            }
        }
